...
function Library:CreateWindow(config)
    ...
    -- Smoothing berbasis Tween, BUKAN RenderStepped. Saat drag/resize aktif,
    -- posisi/ukuran mengikuti input langsung (responsif). Saat touch dilepas,
    -- satu Tween singkat mengantar window ke posisi/ukuran target akhir sehingga
    -- gerakan terasa halus ("mendarat") tanpa listener per-frame yang persisten.
    -- Tween jalan engine-side, sekali per gerakan, lalu auto-cleanup.
    local dragTween = nil
    local resizeTween = nil
    local DRAG_TWEEN_INFO = TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    ...
    self:AddConnection("headerDragStart", scriptHeader.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            bringToFront()
            if dragTween then dragTween:Cancel() dragTween = nil end
    ...
    self:AddConnection("inputEnded", UserInputService.InputEnded:Connect(function(input)
        ...
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            -- Simpan target akhir sebelum flag dimatikan, lalu jalankan Tween
            -- singkat supaya window "mendarat" halus di posisi/ukuran terakhir.
            local finalPos, finalSize = self._win.Position, self._win.Size
            dragging = false
            resizing = false
            releaseMoveConn()
            if finalPos and self._win.Position == finalPos then
                -- posisi tidak berubah lagi, tidak perlu tween
            else
                dragTween = TweenService:Create(self._win, DRAG_TWEEN_INFO, {Position = finalPos})
                dragTween:Play()
            end
            if finalSize and self._win.Size == finalSize then
                -- ukuran tidak berubah lagi, tidak perlu tween
            else
                resizeTween = TweenService:Create(self._win, DRAG_TWEEN_INFO, {Size = finalSize})
                resizeTween:Play()
            end
        end
    end))
