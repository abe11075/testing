...
Library._initSerial = 0





Library._buildActive      = false
Library._buildBudgetUsed  = 0
...
function Library:_queueInitialize(delayTime)
    if self._initialized or not self._pendingWindowObj then return end
    self._initSerial = (self._initSerial or 0) + 1
    local serial = self._initSerial
    if self._initWatcher then
        pcall(function() task.cancel(self._initWatcher) end)
        self._initWatcher = nil
    end
    self._initWatcher = task.delay(delayTime or 0.05, function()
        if Library._initSerial ~= serial or Library._initialized then return end
        Library._initWatcher = nil
        Library:Initialize()
    end)
end
...
function Library:Cleanup()
    ...
    self._dropdownCloseCallback = nil
...
local function ExecuteConfigCallbacks(afterVisual)
    ...
    local visualCount = 0
    local VISUAL_STAGGER_EVERY = 256
...
Library._dropdownCloseCallback = nil
...
function Library:_showDropdown(dropdownContainer, closeCallback)
    self:_initDropdownSystem()
    if self._dropdownCloseCallback and self._dropdownCloseCallback ~= closeCallback then
        pcall(self._dropdownCloseCallback)
    end
    self._dropdownCloseCallback = closeCallback
...
function Library:_hideDropdown()
    local closeCallback = self._dropdownCloseCallback
    self._dropdownCloseCallback = nil
    if closeCallback then
        pcall(closeCallback)
    end
    if self._dropdownOverlay then
        if self._dropdownPanel then
...
function Library:_createBaseDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue, isMulti)
    ...
    local visibleOptions = allOptions
    ...
    local function normalizeOption(opt)
        local label, value = readOption(opt)
        return {
            label = label,
            value = value,
    ...
    local function getOptionData(index)
        local data = allOptions[index]
        if data then return data end
        local raw = items[index]
        if raw == nil then return nil end
        data = normalizeOption(raw)
        allOptions[index] = data
        optionLabelByValue[data.value] = data.label
        return data
    end

    local function getVisibleCount()
        if visibleOptions == filteredOptions then
            return #filteredOptions
        end
        return #items
    end

    local function getVisibleOption(index)
        if visibleOptions == filteredOptions then
            return filteredOptions[index]
        end
        return getOptionData(index)
    end

    local function getLabelForValue(value)
        local label = optionLabelByValue[value]
        if label ~= nil then return label end
        for i = 1, #items do
            local data = allOptions[i]
            if data then
                if data.value == value then
                    return data.label
                end
            else
                local rawLabel, rawValue = readOption(items[i])
                optionLabelByValue[rawValue] = rawLabel
    ...
    local function resetOptionsCache()
    ...
    local function applyFilter(query)
        ...
        if query == lastFilterQuery and visibleOptions == filteredOptions then
            return
        end
        table.clear(filteredOptions)
        for i = 1, #items do
            local opt = getOptionData(i)
            if opt and string.find(opt.lower, query, 1, true) then
    ...
    local function updateClosedLabel()
        if isMulti then
            ...
            if n == 0 then
                ...
                local labels = {}
                for _, v in ipairs(DropdownFunc.Value) do
                    local label = getLabelForValue(v)
                    if label then
            ...
            if v == nil then
                ...
                optionLabel.Text = getLabelForValue(v) or tostring(v)
            end
        end
    end
    ...
    local function refreshVisible()
        ...
        if canvasH ~= lastCanvasHeight then
        ...
        for i = 1, #rowPool do
            ...
            if optIndex <= lastVisible then
                local opt = getVisibleOption(optIndex)
                if opt then
...
local function ensurePoolBuilt()
    ...
    for i = 1, POOL_SIZE do
        ...
        self:AddConnection("dropdownRowHoverOut_" .. dropdownLayoutOrder .. "_" .. i, row.MouseLeave:Connect(function()
            ...
            local opt = optIndex and getVisibleOption(optIndex)
            if opt then
                paintRow(row, opt)
            end
        end))
    ...
    function DropdownFunc:AddOption(option)
        ...
        if visibleOptions == filteredOptions and lastFilterQuery and lastFilterQuery ~= "" then
            local optionData = getOptionData(#items)
            if string.find(optionData.lower, lastFilterQuery, 1, true) then
                filteredOptions[#filteredOptions + 1] = optionData
            end
        end
        queueRefresh()
    end
    ...
    function DropdownFunc:SetValues(newList, selecting)
        ...
        rebuildSelectedSet()
        if isOpen or (isMulti and #DropdownFunc.Value > 0) or (not isMulti and DropdownFunc.Value ~= nil) then
    ...
    function DropdownFunc:Refresh(newList)
        ...
        if isMulti then
            hasSelection = type(prevValue) == "table" and #prevValue > 0
            if hasSelection then
                local stillValid = {}
                if type(prevValue) == "table" then
                    for _, v in ipairs(prevValue) do
                        if getLabelForValue(v) ~= nil then
            ...
            hasSelection = prevValue ~= nil
            if hasSelection then
                local found = getLabelForValue(prevValue) ~= nil
    ...
    self:AddConnection("dropdownOpen_" .. dropdownLayoutOrder, dropdownButton.Activated:Connect(function()
        ...
        scrollSelect.CanvasPosition = Vector2.new(0, 0)
        self:_showDropdown(dropdownContainer, closeCurrentDropdown)
        queueRefresh()
    end))
