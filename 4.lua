...
Library._lowGraphics = _G.LynxLowGraphics ~= false
Library._hoverEffects = _G.LynxHoverEffects == true
Library._showDiscord = _G.LynxShowDiscord == true
Library._searchEnabled = _G.LynxSearchEnabled == true
Library._configTabEnabled = _G.LynxConfigTabEnabled == true
Library._notificationsEnabled = _G.LynxNotifications == true
Library._resizable = _G.LynxResizable == true
...
Library._lastBuildActivity = os.clock() -- di-update tiap AddTab -> dipakai idle-watcher
...
local panelTransparency = Library._lowGraphics and 0 or 0.1
local sectionTransparency = Library._lowGraphics and 0 or 0.30
local fontSize = {
    title = 15,
    subtitle = 11,
    header = 12,
...
-- BUILD_BUDGET_MS = 6ms < 1 frame (16.6ms @60fps) -> tidak ada partial-flash,
-- tapi instance creation tersebar ke beberapa frame sehingga load awal tidak
-- membekukan UI.
local BUILD_BUDGET_MS = 6
local CHECK_EVERY = 16 -- cek waktu tiap 16 instance
...
local function addCorner(props)
    if Library._lowGraphics then return nil end
    return new("UICorner", props)
end
local function addStroke(props)
    if Library._lowGraphics then return nil end
    return new("UIStroke", props)
end
local function addGradient(props)
    if Library._lowGraphics then return nil end
    return new("UIGradient", props)
end
local function useHoverEffects()
    return Library._hoverEffects and not isMobile
end
...
function Library:CreateWindow(config)
    ...
    addCorner({Parent = self._win, CornerRadius = UDim.new(0, 7)})
    ...
    addCorner({Parent = headerDragHandle, CornerRadius = UDim.new(0, 2)})
    new("TextLabel", {
        Parent = scriptHeader,
        Text = title,
    ...
    addCorner({Parent = btnMinHeader, CornerRadius = UDim.new(0, 5)})
    local btnMinStroke = addStroke({
        Parent = btnMinHeader,
        Color = colors.border,
        Thickness = 1,
        Transparency = 0.4
    ...
    if Library._showDiscord then
        local discordLink = "https://discord.gg/lynxx"
        local discordText = "discord.gg/lynxx"
        local discordTextStart = 33
        local discordTextW = 78
        local discordPillW = discordTextStart + discordTextW + 9
        ...
        local discordTitle = new("TextLabel", {
            Parent = btnDiscord,
            Text = discordText,
            Size = UDim2.new(0, discordTextW + 2, 1, 0),
    ...
    local navTop = Library._searchEnabled and 34 or 6
    local navBottom = Library._searchEnabled and 39 or 11
    self._navContainer = new("ScrollingFrame", {
        Parent = self._sidebar,
        Size = UDim2.new(1, -10, 1, -navBottom),
        Position = UDim2.new(0, 5, 0, navTop),
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
    ...
    addCorner({Parent = pageTitleAccent, CornerRadius = UDim.new(1, 0)})
    ...
    if Library._resizable then
        ...
        local function addResizeGripLine(offsetX, offsetY, length)
            ...
            addCorner({Parent = line, CornerRadius = UDim.new(1, 0)})
        end
    ...
    local function createMinimizedIcon()
        ...
        addCorner({Parent = icon, CornerRadius = UDim.new(0, 6)})
        new("TextLabel", {
            Parent = icon,
            Text = "L",
    ...
    local function releaseEndConn()
        if endConn and not dragging and not resizing then
            endConn:Disconnect()
            endConn = nil
        end
    end
    ...
    if Library._searchEnabled then
...
function Library:_createSearchBar()
    ...
    addCorner({Parent = searchContainer, CornerRadius = UDim.new(0, 5)})
    ...
    addCorner({Parent = resultsPanel, CornerRadius = UDim.new(0, 5)})
    addStroke({Parent = resultsPanel, Color = colors.border, Thickness = 1, Transparency = 0.35})
    ...
    local function highlightFeature(frame)
        ...
        if Library._lowGraphics then return end
        local hl = addStroke({
            Parent = frame,
    ...
    local function buildRow()
        ...
        addCorner({Parent = row, CornerRadius = UDim.new(0, 4)})
        ...
        addCorner({Parent = accent, CornerRadius = UDim.new(1, 0)})
...
function Library:CreatePage(name, title, imageId, order)
    ...
    addCorner({Parent = btn, CornerRadius = UDim.new(0, 5)})
    ...
    addCorner({Parent = indicator, CornerRadius = UDim.new(1, 0)})
    local navIcon = nil
    local navLabelX = 8
    local navLabelW = -16
    if imageId and imageId ~= "" and not Library._lowGraphics then
        navIcon = new("ImageLabel", {
            Parent = btn,
            Image = imageId,
            Size = UDim2.new(0, 15, 0, 15),
    ...
    local navLabel = new("TextLabel", {
        Parent = btn,
        Text = name,
        Size = UDim2.new(1, navLabelW, 1, 0),
        Position = UDim2.new(0, navLabelX, 0, 0),
        BackgroundTransparency = 1,
        Font = Enum.Font.GothamBold,
...
function Library:CreateCategory(parent, title, startOpen)
    ...
    addCorner({Parent = categoryFrame, CornerRadius = UDim.new(0, 4)})
...
function Library:CreateToggle(parent, label, configPath, callback, disableSave, defaultValue)
    ...
    addCorner({Parent = toggleBg, CornerRadius = UDim.new(1, 0)})
    ...
    addCorner({Parent = toggleCircle, CornerRadius = UDim.new(1, 0)})
...
function Library:_initDropdownSystem()
    ...
    addCorner({Parent = self._dropdownPanel, CornerRadius = UDim.new(0, 6)})
    addStroke({
...
function Library:_createBaseDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue, isMulti)
    ...
    addCorner({Parent = selectFrame, CornerRadius = UDim.new(0, 4)})
    addStroke({Parent = selectFrame, Color = colors.border, Thickness = 1, Transparency = 0.5})
    ...
    if not Library._lowGraphics then
        new("ImageLabel", {
            Parent = selectFrame,
    ...
    local function ensureDropdownBuilt()
        if dropdownBuilt then return end
        dropdownBuilt = true
        self:_initDropdownSystem()
        dropdownContainer = new("Frame", {
            Parent = self._dropdownFolder,
            Size = UDim2.new(1, 0, 1, 0),
            BackgroundTransparency = 1,
            LayoutOrder = dropdownLayoutOrder
        ...
        addCorner({Parent = searchBox, CornerRadius = UDim.new(0, 4)})
        addStroke({Parent = searchBox, Color = colors.border, Thickness = 1, Transparency = 0.5})
        new("UIPadding", {Parent = searchBox, PaddingLeft = UDim.new(0, 8)})
        scrollSelect = new("ScrollingFrame", {
            Parent = dropdownContainer,
        ...
        self:AddConnection("searchBox_" .. dropdownLayoutOrder, searchBox:GetPropertyChangedSignal("Text"):Connect(function()
            ...
            currentQuery = string.lower(searchBox.Text or "")
            if currentQuery == "" then
                applyFilter("")
                if scrollSelect then scrollSelect.CanvasPosition = Vector2.new(0, 0) end
                refreshVisible()
                return
            end
    ...
    refreshVisible = function()
        if not poolBuilt or not scrollSelect then return end
...
local function ensurePoolBuilt()
    ...
    for i = 1, POOL_SIZE do
        ...
        addCorner({Parent = row, CornerRadius = UDim.new(0, 3)})
        new("TextLabel", {
            Parent = row,
            Font = Enum.Font.GothamBold,
            Text = "",
        ...
        addCorner({Parent = chooseFrame, CornerRadius = UDim.new(0, 3)})
        local btn = new("TextButton", {
            Parent = row,
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 1, 0),
...
function DropdownFunc:Clear()
    ...
    function DropdownFunc:AddOption(option)
        ...
        if currentQuery == "" or string.find(allOptions[#allOptions].lower, currentQuery, 1, true) then
...
function Library:CreateInput(parent, label, configPath, defaultValue, callback)
    ...
    addCorner({Parent = inputBg, CornerRadius = UDim.new(0, 4)})
...
function Library:CreateButton(parent, label, callback)
    ...
    addCorner({Parent = btnFrame, CornerRadius = UDim.new(0, 5)})
    ...
    addCorner({Parent = button, CornerRadius = UDim.new(0, 5)})
...
function Library:Initialize()
    ...
    if self._pendingWindowObj then
        if Library._configTabEnabled then
...
function Library:MakeNotify(config)
    if not Library._notificationsEnabled then return end
    config = config or {}
    ...
    addCorner({Parent = notif, CornerRadius = UDim.new(0, 6)})
    local accent = new("Frame", {
        Parent = notif,
        Size = UDim2.new(0, 3, 1, -8),
        Position = UDim2.new(0, 4, 0, 4),
...
function Library:Window(config)
    ...
    -- Idle-watcher: jalankan Initialize ~150ms SETELAH AddTab terakhir (build
    -- benar-benar idle), bukan timer fixed 0.5s. Otomatis menyesuaikan durasi
    -- build berapa pun. Aman walau build yielding: watcher polling tiap frame
    -- dan baru menyalakan callback ketika AddTab sudah 100ms tidak dipanggil,
    -- jadi tidak pernah menjalankan callback sebelum seluruh UI selesai dibangun.
    local IDLE_THRESHOLD = 0.10 -- 100ms tanpa AddTab baru = build dianggap selesai
    local EXTRA_DELAY    = 0.05 -- kasih 50ms ekstra setelah idle biar layout settle
    ...
    function WindowObject:AddTab(tabConfig)
        ...
        local iconId = ""
        if tabIcon and tabIcon ~= "" and not Library._lowGraphics then
        ...
        function TabObject:AddSection(sectionTitle, isOpen)
            ...
            local function registerFeature(featureName, featureFrame, featureKind)
                if not Library._searchEnabled or not featureName or not featureFrame then return end
            ...
            function SectionObject:AddParagraph(paragraphConfig)
                ...
                addCorner({Parent = frame, CornerRadius = UDim.new(0, 5)})
                addStroke({Parent = frame, Color = colors.border, Thickness = 1, Transparency = 0.65})
                new("UIPadding", {
                    Parent = frame,
