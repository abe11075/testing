...
function Library.ConfigSystem.Save()
    ...
    local ok, err = pcall(function()
        ...
        if response and response.StatusCode ~= 200 then
            warn("[LynxCloud] Save Error:", response.StatusCode, response.Body)
        end
    end)
    if not ok then warn("[LynxCloud] Exception in Save:", err) end
    return ok
end

function Library.ConfigSystem.Load()
    CurrentConfig = DeepCopy(DefaultConfig)
    if not _G.LynxCloudURL or not _G.LynxCloudSecret or not http_request then return CurrentConfig end
    local ok, err = pcall(function()
        local baseUrl = _G.LynxCloudURL:gsub("/$", "")
        local url = baseUrl .. "/config?uid=" .. tostring(localPlayer.UserId)
        local response = http_request({
    ...
    if not ok then warn("[LynxCloud] Exception in Load:", err) end
