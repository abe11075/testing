...
function Library:_createBaseElement(parent, label, height, labelSize)
	...
	if label then
		textLabel = new("TextLabel", {
			Parent = frame,
			Text = label,
			Size = labelSize or UDim2.new(1, -45, 1, 0),
			BackgroundTransparency = 1,
...
function Library:_addCorner(parent, radius)
	return new("UICorner", { Parent = parent, CornerRadius = UDim.new(0, radius or 5) })
end
...
function Library:Window(config)
	...
	function WindowObject:AddTab(tabConfig)
		...
		function TabObject:AddSection(sectionTitle, isOpen)
			...
			SectionObject._tabName = tabName
