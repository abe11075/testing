...
local BUILD_BUDGET_MS = 6 -- yield 1 frame kalau sudah segini ms dipakai sejak yield terakhir.
-- 6ms < 1 frame (16.6ms @60fps) -> tidak ada partial-flash, tapi instance creation
-- tersebar ke beberapa frame sehingga load awal tidak membekukan UI.
local function maybeYieldBuild()
    if not Library._buildActive then return end
    local now = os.clock()
    local used = now - Library._buildLastYield
    if used >= BUILD_BUDGET_MS then
        Library._buildLastYield = now
        task.wait() -- yield 1 frame; engine render, lalu lanjut build. State lokal
        -- pemanggil tetap utuh (yield di Lua tidak menghapus upvalue/lokal), dan
        -- return value new() tetap jalan karena maybeYield dipanggil SEBELUM return.
    end
end
...
function Library:CreateWindow(config)
    ...
    -- Window stroke: garis tepi tipis semi-transparan dengan gradient vertikal.
    -- Memberi kesan jendela "melayang" terdefinisi seperti UI modern, tanpa
    -- border keras. Ringan: 1 UIStroke statis, no per-frame.
    ...
    -- Brand chip: badge bulat kecil di kiri header dengan logo di dalamnya.
    -- Menggantikan logo polos lama. Tampak seperti ikon app premium: ada
    -- background tipis + stroke berwarna primary + icon ter-tint. 3 instance
    -- statis, nol per-frame.
    local brandChip = new("Frame", {
        Parent = scriptHeader,
    ...
    new("UICorner", {Parent = brandChip, CornerRadius = UDim.new(0, 7)})
    new("UIStroke", {
        Parent = brandChip,
        Color = colors.primary,
        Thickness = 1,
    ...
    -- Title bergeser ke kanan chip. Tiap device padding konsisten.
    new("TextLabel", {
        Parent = scriptHeader,
        Text = title,
    ...
    local separator = new("Frame", {
        ...
        Position = UDim2.new(0, 110, 0.5, -8),
        BackgroundColor3 = colors.border,
