...
function Library:CreateWindow(config)
    ...
    self._win = new("Frame", {
        ...
        -- Opaque (0): frame semi-transparan memaksa alpha compositing tiap frame
        -- saat posisi/ukuran berubah (drag/resize), bikin gerakan kasar di mobile.
    ...
    -- Smoothing untuk mobile (Delta Android punya touch sampling rate rendah:
    -- posisi input loncat-loncat). Daripada set posisi/ukuran window langsung
    -- dari input jittery, kita simpan "target" saat InputChanged lalu lerp
    -- window ke target tiap RenderStepped. Window mengejar pointer secara halus.
    local targetPos = nil
        ...
        lerpConn = RunService.RenderStepped:Connect(function()
            if targetPos then
                ...
                self._win.Position = UDim2.new(
                    cur.X.Scale, cur.X.Offset + (targetPos.X.Offset - cur.X.Offset) * 0.45,
                    cur.Y.Scale, cur.Y.Offset + (targetPos.Y.Offset - cur.Y.Offset) * 0.45
                )
            end
            if targetSize then
                local cur = self._win.Size
                self._win.Size = UDim2.new(
                    0, cur.X.Offset + (targetSize.X.Offset - cur.X.Offset) * 0.45,
                    0, cur.Y.Offset + (targetSize.Y.Offset - cur.Y.Offset) * 0.45
                )
            end
        end)
    ...
    local function releaseLerpConn()
        if lerpConn and not dragging and not resizing then
            lerpConn:Disconnect()
            lerpConn = nil
            targetPos = nil
            targetSize = nil
    ...
    self:AddConnection("inputEnded", UserInputService.InputEnded:Connect(function(input)
        ...
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            resizing = false
            releaseMoveConn()
            -- Beri jeda singkat sebelum lepaskan lerp, supaya window sempat
            -- "mendarat" halus ke posisi target terakhir sebelum koneksi mati.
...
function Library:CreatePage(name, title, imageId, order)
    ...
    local contentContainer = new("ScrollingFrame", {
        ...
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        -- ScrollBar tipis + warna tema: di mobile, scrollbar terlihat (meskipun
        -- tipis) memicu engine apply momentum/inertia scroll yang mulus. Tanpa
        -- scrollbar (thickness 0), scroll touch terasa kaku/kasar.
