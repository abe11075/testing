...
Library._LoadSuccess = false
Library._IsSaving = false

function Library.ConfigSystem.Save()
    if not Library._LoadSuccess then
        warn("[LynxCloud] Save aborted: Config was never loaded successfully! (Safe Lock)")
        return false
    end
    if Library._IsSaving then
        return false -- Prevent overlapping saves
    end
    if not _G.LynxCloudURL or not _G.LynxCloudSecret then
        warn("[LynxCloud] Missing _G.LynxCloudURL or _G.LynxCloudSecret!")
        return false
    end
    if not http_request then
        warn("[LynxCloud] Executor does not support HTTP requests!")
        return false
    end
    
    Library._IsSaving = true
    local baseUrl = _G.LynxCloudURL:gsub("/$", "")
    local url = baseUrl .. "/config?uid=" .. tostring(localPlayer.UserId)
    ...
    task.spawn(function()
            ...
            local ok, err = pcall(function()
                local response = http_request({
                    ...
                    Headers = {
                        ...
                        ["x-lynx-secret"] = _G.LynxCloudSecret
                    },
                    Body = body
                })
                if response and response.StatusCode == 200 then
                    ...
                    warn("[LynxCloud] Save Error (Attempt " .. attempt .. "):", response and response.StatusCode or "Network Error")
                end
            end)
        ...
        if not success then
            warn("[LynxCloud] CRITICAL: Save failed after 3 attempts!")
        end
        Library._IsSaving = false
    end)
    return true
...
function Library.ConfigSystem.Load()
    ...
    Library._LoadSuccess = false
    if not _G.LynxCloudURL or not _G.LynxCloudSecret or not http_request then 
        warn("[LynxCloud] Cannot load: Missing credentials or HTTP support.")
    ...
    if not Library._LoadSuccess then
        warn("[LynxCloud] CRITICAL: Failed to load config after " .. maxRetries .. " attempts. Auto-save is DISABLED to protect data.")
    end
...
local function MarkDirty()
    ...
    Library._saveThread = task.delay(5, function()
        if not isDirty then
            Library._saveThread = nil
            return
        end
        local ok, started = pcall(function() return Library.ConfigSystem.Save() end)
        if ok and started then
            isDirty = false
        else
            -- Save was locked/aborted (isSaving), keep isDirty true and retry
            task.delay(0.1, function() MarkDirty() end)
        end
        Library._saveThread = nil
    end)
end
...
function Library.ConfigSystem.Delete()
    if not _G.LynxCloudURL or not _G.LynxCloudSecret or not http_request then return end
    pcall(function()
        local baseUrl = _G.LynxCloudURL:gsub("/$", "")
        local url = baseUrl .. "/config?uid=" .. tostring(localPlayer.UserId)
        local response = http_request({
        ...
        if response and response.StatusCode ~= 200 then
            warn("[LynxCloud] Delete Error:", response.StatusCode, response.Body)
        end
    end)
end
...
function Library.ConfigSystem.Import(username)
    ...
    if not _G.LynxCloudURL or not _G.LynxCloudSecret or not http_request then 
        Library:MakeNotify({ Title = "Import Gagal", Description = "Koneksi ke server tidak tersedia.", Delay = 3 })
        return 
    end
    ...
    local baseUrl = _G.LynxCloudURL:gsub("/$", "")
    local url = baseUrl .. "/config/import?username=" .. urlEncode(username)
    ...
    task.spawn(function()
        local ok, err = pcall(function()
            ...
            if response and response.StatusCode == 200 and response.Body then
                local decoded = HttpService:JSONDecode(response.Body)
                if decoded and decoded.config and type(decoded.config) == "table" then
                    -- Hapus semua settingan lama (Reset ke Default)
                    CurrentConfig = DeepCopy(DefaultConfig)
                    -- Eksekusi callback dengan settingan default untuk "mematikan" (turn off) 
                    -- fungsi-fungsi yang sedang berjalan (mencegah bug double loop)
                    ExecuteConfigCallbacks()
                    
                    Library:MakeNotify({ Title = "Importing...", Description = "Mereset fungsi lama (Harap tunggu)...", Delay = 2 })
                    
                    -- Beri jeda lebih lama (2 detik) agar loop/thread di script utama 
                    -- benar-benar terhenti sepenuhnya, terutama di device lambat seperti Redfinger
                    task.wait(2)
                    
                    -- Sekarang gabungkan murni dengan config yang di-import
                    MergeTables(CurrentConfig, decoded.config)
                    
                    -- Jalankan callback lagi untuk menyalakan fitur sesuai config baru
                    ExecuteConfigCallbacks()
                    -- Paksa simpan ke database pemain (Auto-Save)
                    pcall(MarkDirty)
                    
                    Library:MakeNotify({ Title = "Import Berhasil!", Description = "Berhasil menyalin config milik " .. username, Delay = 4 })
                end
            elseif response and response.StatusCode == 404 then
                Library:MakeNotify({ Title = "Import Gagal", Description = "Username '" .. username .. "' tidak ditemukan.", Delay = 3 })
            else
...
function Library:_createConfigTab(WindowObject)
    ...
    local importSection = configTab:AddSection("Import Config")
    local importTarget = ""
    importSection:AddInput({
        Title       = "Roblox Username",
        Placeholder = "Ketik username...",
        NoSave      = true,
        Callback    = function(text)
            importTarget = text or ""
        end,
    })
    importSection:AddButton({
        Title = "Import Now",
        Callback = function()
            if importTarget and importTarget ~= "" then
                Library.ConfigSystem.Import(importTarget)
            else
                self:MakeNotify({ Title = "Import", Description = "Masukkan username terlebih dahulu!", Delay = 2 })
            end
        end,
    })
