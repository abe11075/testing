    ...
    task.spawn(function()
        local ok, err = pcall(function()
            ...
            if response and response.StatusCode == 200 and response.Body then
                local decoded = HttpService:JSONDecode(response.Body)
                if decoded and decoded.config and type(decoded.config) == "table" then
                    -- Hapus semua settingan lama dan ganti murni dengan yang di-import
                    CurrentConfig = DeepCopy(DefaultConfig)
                    MergeTables(CurrentConfig, decoded.config)
                    
                    -- Jalankan callback agar visual UI langsung berubah
                    ExecuteConfigCallbacks()
                    -- Paksa simpan ke database pemain (Auto-Save)
                    local function MarkDirtyGlobal()
                        if _G.AutoSaveEnabled == false then return end
                        isDirty = true
                        if Library._saveThread then
                            pcall(function() task.cancel(Library._saveThread) end)
                        ...
                        Library._saveThread = task.delay(5, function()
                            ...
                            if ok and started then
                                ...
                                task.delay(0.1, function() MarkDirtyGlobal() end)
                            end
                            Library._saveThread = nil
                        end)
                    end
                    MarkDirtyGlobal() -- Gunakan logika MarkDirty
