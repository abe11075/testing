...
function Library:Cleanup()
    ...
    if self._activeNotifs then
        for _, notif in ipairs(self._activeNotifs) do
            pcall(function()
                -- Cancel the delayed-destroy thread explicitly so it doesn't
                -- fire later as a (guarded) no-op after Cleanup.
                if notif and notif._delayThread then
                    pcall(function() task.cancel(notif._delayThread) end)
                    notif._delayThread = nil
                end
...
local function ExecuteConfigCallbacks()
    ...
    for _, entry in pairs(CallbackRegistry) do
        if entry.updateVisual then
            -- pcall Get + updateVisual per entry: a corrupt path or a faulty
            -- updateVisual on ONE component must not halt the restore loop for
            -- every other component.
            local ok, value = pcall(Library.ConfigSystem.Get, entry.path, entry.default)
            if ok then
    ...
    --
    -- Heavy callbacks (auto-farm loops, HTTP, raycast) are dispatched via
    -- task.spawn so they run on their own threads instead of blocking the
    -- 0.5s startup window and freezing the first-frame UI render. A simple
    -- semaphore caps concurrent spawned callbacks to avoid spawning dozens of
    -- loops simultaneously on first execute (which can spike memory / get
    -- throttled by the scheduler).
    local MAX_PARALLEL = 8
    local inflight = 0
    local function dispatch(entry)
        local ok, value = pcall(Library.ConfigSystem.Get, entry.path, entry.default)
        if not ok or not entry.callback then return end
        inflight = inflight + 1
        task.spawn(function()
            pcall(entry.callback, value)
            inflight = inflight - 1
        end)
        -- Cheap throttle: if too many heavy callbacks are already running,
        -- yield briefly so we don't queue hundreds of task.spawn threads at
        -- once. The caller's own thread yields here, not the spawned work.
    ...
    runCallbacks(false)
    runCallbacks(true)
    -- Wait for the last wave of spawned callbacks to start before returning,
    -- so Initialize's subsequent steps don't race them unnecessarily.
    while inflight > 0 do
        task.wait(0.05)
    end
end
...
function Library:CreateWindow(config)
    ...
    -- Clear connections from a prior execute eagerly. Previously this relied
    -- solely on the old gui's Destroying event firing Cleanup(); some executors
    -- don't fire Destroying reliably, which leaked stale connections into the
    -- new session. Disconnect & drop them here unconditionally so re-execute
    -- always starts from a clean connection set.
    if self._connections then
        for _, conn in pairs(self._connections) do
            pcall(function() conn:Disconnect() end)
        end
    ...
    if existingGUI then
        ...
        -- No task.wait here: the 100ms yield delayed every re-execute for no
        -- functional gain. Destroy() is synchronous within the data model.
    end
    ...
    -- pcall-guard TextService: GetTextSize can throw on some executors and
    -- would abort the whole window build mid-construction. Fall back to a
    -- measured width estimate so the discord pill still renders.
    local discordTextW = 90
    pcall(function()
        discordTextW = game:GetService("TextService"):GetTextSize(discordText, fontSize.small, Enum.Font.GothamBold, Vector2.new(1000, 100)).X
    end)
    ...
    local function ensureMoveConn()
        ...
        -- InputEnded only matters WHILE dragging/resizing. Previously it was
        -- connected permanently at window creation, which fired the Lua handler
        -- on every key press / mouse button / touch game-wide (the body just
        -- early-returned, but the invocation itself is constant overhead).
        -- Connect lazily here, alongside the move handler, and disconnect when
        -- idle via releaseMoveConn so the cost is paid only during an active
        -- drag/resize session.
        if not endConn then
    ...
    -- InputEnded is no longer connected permanently here; it is connected
    -- lazily by ensureMoveConn() during an active drag/resize and disconnected
    -- by releaseMoveConn() when idle. See the comment above ensureMoveConn.
...
function Library:_initDropdownSystem()
    ...
    -- Shared close handler. Previously every dropdown registered its OWN
    -- connection on this same overlay's Visible property (N connections),
    -- so opening/closing ANY dropdown fired N Lua callbacks -- O(N) per
    -- open/close. Instead, ONE shared connection here calls whatever the
    -- currently-open dropdown registered as its close callback. Each
    -- dropdown just sets self._activeDropdownClose on open; the work is O(1).
    self._activeDropdownClose = nil
    self:AddConnection("dropdownOverlayVisibleChanged", self._dropdownOverlay:GetPropertyChangedSignal("Visible"):Connect(function()
        if not self._dropdownOverlay.Visible then
            local cb = self._activeDropdownClose
            self._activeDropdownClose = nil
...
function Library:_createBaseDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue, isMulti)
    ...
    local function findLabelInItems(value)
        -- O(n) scan over raw items for ONE value, used only when the dropdown
        -- is still closed and the option list has not been normalized yet.
        -- This is dramatically cheaper than normalizeListInto + applyFilter
        -- (which allocate a new table per option + string.lower per option for
        -- the ENTIRE list) -- it does at most one label resolution.
    ...
    local function updateClosedLabel()
        if not optionsNormalized then
            -- Lazy path: dropdown not yet opened. Resolve the closed label from
            -- the raw items list instead of forcing full normalization. This
            -- keeps per-dropdown construction cost O(1) instead of O(total
            -- options). Full normalization still happens on first open via
            -- ensurePoolBuilt -> ensureOptionsNormalized.
            if isMulti then
                ...
                if v == nil then
                    ...
                    optionLabel.Text = findLabelInItems(v) or tostring(v)
                end
            end
            return
        end
        -- Normalized path (dropdown opened at least once): use the indexed
        -- allOptions table as before for consistent behavior.
        if isMulti then
...
function DropdownFunc:Clear()
        allOptions = {}
        filteredOptions = {}
        -- items mirrors allOptions so the lazy label path stays consistent.
        items = {}
        -- Mark normalized so updateClosedLabel uses allOptions (now empty)
        -- instead of falling back to the raw items list.
        optionsNormalized = true
    ...
    function DropdownFunc:AddOption(option)
        ...
        -- Once options are added directly to allOptions, treat the list as
        -- normalized so updateClosedLabel reads from allOptions (which now
        -- includes this option) rather than the stale raw items list.
        optionsNormalized = true
        ...
        -- Keep items in sync for any caller that inspects DropdownFunc.Options
        -- or for code paths that still reference the raw list.
    ...
    self:AddConnection("dropdownOpen_" .. dropdownLayoutOrder, dropdownButton.Activated:Connect(function()
        ...
        scrollSelect.CanvasPosition = Vector2.new(0, 0)
        isOpen = true
        -- Register this dropdown as the active one so the single shared
        -- overlay Visible-changed connection (set up once in _initDropdownSystem)
        -- can clear our isOpen flag on close. Replaces the old per-dropdown
        -- connection that fired for every dropdown on every open/close.
        self._activeDropdownClose = function()
            if isOpen then isOpen = false end
        end
...
function Library:CreateInput(parent, label, configPath, defaultValue, callback)
    ...
    self:AddConnection("input_" .. label .. tostring(inputBox), inputBox.FocusLost:Connect(function()
        ...
        -- pcall-guard user callback: a throw here would otherwise leak the
        -- FocusLost connection handler in a broken state and, depending on the
        -- executor, abort the surrounding input handling.
    ...
    if callback then
        ...
        -- Spawn + pcall: this fires during construction (and again later in
        -- ExecuteConfigCallbacks). Input callbacks are frequently heavy
        -- (HTTP/raycast/long loops), and running them synchronously on the
        -- build thread (a) freezes the first-frame UI render and (b) lets one
        -- bad input abort the rest of the section build. task.spawn detaches
        -- it onto its own thread so the UI appears immediately and other
        -- components keep building; pcall isolates any error.
        task.spawn(function()
...
function Library:Initialize()
    ...
    -- pcall-guard ExecuteConfigCallbacks: although individual callbacks are
    -- already pcall'd, the dispatch loop itself (or ConfigSystem.Get on a bad
    -- path) could throw. Previously a throw here skipped the PlayerRemoving
    -- connection below, silently disabling autosave-on-leave. The 0.5s guard
    -- in Window() also won't retry because _initialized is already true.
    pcall(ExecuteConfigCallbacks)
    ...
    -- Always connect PlayerRemoving last and outside any pcall, so autosave-on-
    -- leave stays functional even if config dispatch or the Config tab threw.
...
function Library:MakeNotify(config)
    ...
    notif._delayThread = task.delay(delay, function()
        pcall(function()
    ...
    notif.Destroying:Connect(function()
        -- Cancel the pending delayed-destroy thread so it doesn't linger as a
        -- no-op after Cleanup. The thread's body guards notif.Parent so this
        -- is housekeeping, not correctness, but cancelling keeps the thread
        -- count bounded across re-executes.
        if notif._delayThread then
            pcall(function() task.cancel(notif._delayThread) end)
            notif._delayThread = nil
        end
...
function Library:Window(config)
    ...
    -- pcall-guard config load: EnsureFolderExists/makefolder/readfile can
    -- throw on sandboxed executors. Previously a throw here aborted Window()
    -- AFTER the GUI shell was already parented -- the window showed up but no
    -- tabs were built and the script silently stopped. Falling back to
    -- DefaultConfig (already what Load() does internally on decode failure)
    -- keeps the rest of startup alive.
