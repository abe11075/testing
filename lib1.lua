...
function Library:CreateWindow(config)
    ...
    -- DUA masalah mobile (Delta Android), dua solusi yang saling lengkapi:
    --  1) Transparansi window saat bergerak = alpha compositing tiap frame -> FPS
    --     anjlok -> drag ngaret. Fix: window OPAQUE saat drag, transparan saat diam.
    --  2) Touch sampling rate Delta rendah -> posisi input loncat-loncat -> window
    --     ikut loncat (sendat). Fix: simpan TARGET dari input, lalu lerp window ke
    --     target tiap RenderStepped (lerp = matematika murah, bukan penyebab FPS
    --     drop -- transparansi itu). Window "mengalir" halus mengejar jari.
    -- RenderStepped hanya connect saat drag aktif, disconnect saat dilepas, jadi
    -- idle = nol loop per-frame.
    local savedWinTransparency = panelTransparency
    local targetPos = nil
    local targetSize = nil
    local lerpConn = nil
    -- Cache window di upvalue sekali per drag, jadi loop gak lookup self._win
    -- (table access) tiap frame.
    local win = self._win
    local function ensureLerpConn()
        if lerpConn then return end
        -- Factor 0.6: snappy tapi tetap haluskan jitter Delta. SKIP frame saat
        -- sudah "sampai" (delta < 4px) dan SNAP langsung ke target supaya window
        -- bener-bener berhenti nulis -> engine gak repaint properti tiap frame
        -- padahal nilai udah sama -- itulah sumber rasa "berat".
        lerpConn = RunService.RenderStepped:Connect(function()
            if targetPos then
                ...
                if dx*dx + dy*dy > 16 then
                    win.Position = UDim2.new(cur.X.Scale, cur.X.Offset + dx * 0.6, cur.Y.Scale, cur.Y.Offset + dy * 0.6)
            ...
            if targetSize then
                local cur = win.Size
                local dx = targetSize.X.Offset - cur.X.Offset
                local dy = targetSize.Y.Offset - cur.Y.Offset
                if dx*dx + dy*dy > 16 then
                    win.Size = UDim2.new(0, cur.X.Offset + dx * 0.6, 0, cur.Y.Offset + dy * 0.6)
                elseif dx ~= 0 or dy ~= 0 then
                    win.Size = targetSize
                end
            end
        end)
        self:AddConnection("lerpUpdate", lerpConn)
    end
    local function releaseLerpConn()
        if lerpConn then
            lerpConn:Disconnect()
            lerpConn = nil
        end
    ...
    self:AddConnection("headerDragStart", scriptHeader.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            targetPos = self._win.Position
            -- Opaque saat drag: nol biaya alpha compositing -> FPS stabil.
            savedWinTransparency = self._win.BackgroundTransparency
    ...
    self:AddConnection("inputEnded", UserInputService.InputEnded:Connect(function(input)
        ...
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            resizing = false
            releaseMoveConn()
            -- Hentikan smoothing setelah jeda singkat supaya window sempat "mendarat"
            -- halus di target akhir sebelum loop dimatikan.
            task.delay(0.12, function()
                if not dragging and not resizing then releaseLerpConn() end
            end)
            -- Kembalikan transparansi: window diam lagi, alpha compositing murah.
            self._win.BackgroundTransparency = savedWinTransparency
        end
    end))
...
function Library:_createBaseDropdown(parent, title, imageId, items, configPath, onSelect, uniqueId, defaultValue, isMulti)
    ...
    function DropdownFunc:AddOption(option)
        ...
        -- UIStroke dihapus dari tiap opsi: untuk dropdown ratusan item itu ratusan
        -- pass render ekstra saat scroll. Diferensiasi selected/unselected sudah
        -- cukup jelas lewat BackgroundColor3 + accent ChooseFrame di paintOption.
        ...
        -- UIStroke ChooseFrame dihapus: saat unselected size=0 (tidak kelihatan),
        -- saat selected warna primary sudah cukup tajam. Menghemat 1 pass render
        -- per opsi (total ratusan untuk dropdown besar).
