...
function Library:CreateWindow(config)
    ...
    local function ensureLerpConn()
        ...
        lerpConn = RunService.RenderStepped:Connect(function()
            if targetPos then
                local cur = self._win.Position
                self._win.Position = UDim2.new(
                    cur.X.Scale, cur.X.Offset + (targetPos.X.Offset - cur.X.Offset) * 0.5,
                    cur.Y.Scale, cur.Y.Offset + (targetPos.Y.Offset - cur.Y.Offset) * 0.5
                )
            end
            if targetSize then
                local cur = self._win.Size
                self._win.Size = UDim2.new(
                    0, cur.X.Offset + (targetSize.X.Offset - cur.X.Offset) * 0.5,
                    0, cur.Y.Offset + (targetSize.Y.Offset - cur.Y.Offset) * 0.5
                )
            end
        end)
